...
// التحقق من الفلترة بناءً على اسم الكافل
if (!empty($_GET['sponsor_name'])) {
    $whereClauses[] = "p.sponsor_name LIKE ?";
    $params[] = '%' . $_GET['sponsor_name'] . '%';
    $types .= "s";
}
...
$query = "SELECT p.id, b.name, p.amount, p.payment_date, p.sponsor_name 
          FROM payments p 
    ...
    <div class="card card-elevated">
        <div class="card-body">
            ...
            <form method="GET" class="mb-4 row g-3">
                ...
                <div class="col-md-3">
                    <input type="text" name="sponsor_name" class="form-control" placeholder="🔍 فلترة باسم الكافل" value="<?php echo isset($_GET['sponsor_name']) ? $_GET['sponsor_name'] : ''; ?>"> <!-- فلترة باسم الكافل -->
                </div>
                <div class="col-md-2">
            ...
            <table class="table table-striped table-modern m-0">
                <thead>
                    <tr>
                        <th>الاسم</th>
                        <th>اسم الكافل</th> <!-- عمود جديد لاسم الكافل -->
                        <th>المبلغ</th>
                        <th>تاريخ الدفع</th>
                        <th>ملاحظات</th>
                    </tr>
                ...
                <tbody>
                    ...
                    ?>
                        <tr>
                            ...
                            <td><?php echo $payment['sponsor_name']; ?></td> <!-- عرض اسم الكافل -->
                            <td><?php echo number_format($payment['amount'], 2); ?></td>
                            <td><?php echo $payment['payment_date']; ?></td>
                            <td></td> <!-- لا توجد ملاحظات هنا -->
                        </tr>
                    <?php 
                        }
                    } else {
                        echo "<tr><td colspan='5' class='text-center'>لا توجد نتائج مطابقة</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
